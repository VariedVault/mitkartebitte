...
// Generic Cases & Grammar tier home - the grammar analogue of the verb level page, for any
// tier. A1 keeps its own dedicated home (grammarA1.js) untouched; this drives A2 (and later
// B1) from the same shared data layer, so a new tier is data + one route, no new home code.
const TIER_TITLES = {
  A2: 'Two-way Prepositions & Adjectives',
  B1: 'Declensions & Relative Pronouns',
};

export function renderGrammarTierHome(container, { profileId, tier, setBreadcrumb }) {
  const tierPath = `/cases/${tier.toLowerCase()}`;
  setBreadcrumb(`Learn · ${tier} Cases & Grammar`);
  container.innerHTML = '';
  container.appendChild(backLink('Learn', () => navigate('')));

  container.appendChild(el('h1', {}, `${tier} · ${TIER_TITLES[tier] || 'Cases & Grammar'}`));
  ...
  summaryText.appendChild(
    el('p', { style: 'margin:4px 0 0;color:var(--ink-soft);font-size:13px' },
      passed
        ...
        : 'Pass the checkpoint to add these to Grammar Practice (A1 + this tier, cumulative).')
  );
  ...
  if (passed) {
    ...
    practiceBtn.addEventListener('click', () => { store.setSetting(profileId, 'practiceDeck', 'grammar'); navigate('/practice'); });
  ...
  checkpointBtn.addEventListener('click', () => navigate(`${tierPath}/checkpoint`));
  ...
  for (const lesson of lessons) {
    ...
    lc.addEventListener('click', () => navigate(`${tierPath}/lesson/${lesson.id}`));
